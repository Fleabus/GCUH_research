...
class NeuralNetwork:
    ...
    def getY(self, shape):
        output = self.x
        self.conv_layer([1, 5, 1, 32])
        for i in range(0, len(self.shape)):
            output = self.hidden_layer(output, shape[i])
        return output

    def conv_layer(self, layerShape):
        w = self.weight_variable(layerShape)
        b = self.bias_variable(layerShape)

    def conv1d(self, x, W):
        return tf.nn.conv2d(x, W, stride=2, padding='SAME')

    def max_pool_1x2(self, x):
        return tf.nn.max_pool(x, ksize=[1, 1, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
...
with tf.Session() as sess:
    nn.setupModel(sess, [[900, 100], [100, 2]])
